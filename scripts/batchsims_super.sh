...
declare -i k0=103 #start counter of folders
declare -i k=$k0
...
for (( l = 0 ; l < ${#LL[@]} ; l++ )) ; do
    for (( m = 0 ; m < ${#kk[@]} ; m++ )) ; do
        for (( n = 0 ; n < ${#ff[@]} ; n++ )) ; do
          ...
          # run Dedalus script
          mpiexec -n $ncores --oversubscribe python3 $RUNSCRIPT ${LL[$l]} ${kk[$m]} ${ff[$n]} ${facfac[$o]} data/$SESNAME$k $isInertial > $SESNAME$k'.out' 2>&1
