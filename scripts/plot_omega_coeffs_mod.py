...
with writer.saving(fig, "%s/sphere%i_om_coeffs.mp4" %(output_folder, sim_number), dpi):
    ...
    for ind in range(first_frame + comm.rank + 1, last_frame + 1, step):
        ...
        if ind == first_frame + comm.rank +1:
            ...
            ax[0].set_ylabel(r'$\theta$', usetex=True), ax[0].set_xlabel(r'$\phi$', usetex=True)
            ax[0].set_title(r'$\zeta (\theta, \phi)$',fontsize=15, usetex=True)
            ax[0].invert_yaxis()
            ...
            ax[1].set_xlabel('$m$', usetex=True), ax[1].set_ylabel(r'$\ell$', usetex=True)
            ax[1].set_title(r'$\hat{\zeta}_{\ell, m}$',fontsize=15, usetex=True)
            ax[1].set_aspect('equal')
            image1.set_clim(0, 2*np.pi)
            cb = fig.colorbar(image1, ax=ax[1])
