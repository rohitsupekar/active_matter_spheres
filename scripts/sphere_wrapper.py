...
class Sphere:
    def __init__(self,L_max,S_max=0,m_min=None,m_max=None):
        ...
        cos_grid,self.weights = sph.quadrature(self.L_max,niter=3,report_error=False)
        self.grid = np.arccos(cos_grid)
        ...
        for s in range(-S_max,S_max+1):
            for m in range(m_min,m_max+1):
                Y = sph.Y(self.L_max,m,s,cos_grid).astype(np.float64)
                self.pushY[(m,s)] = self.weights*Y
                self.pullY[(m,s)] = Y.T
